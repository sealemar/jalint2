 ...
 *  one 8-bit char, one 32-bit int, one 64-bit double, one 32-bit
 *  float,
...
/**
 *  <i>Binary standard output</i>. This class provides methods for
 *  converting
 *  primtive type variables (<tt>boolean</tt>, <tt>byte</tt>,
 *  <tt>char</tt>,
 *  <tt>int</tt>, <tt>long</tt>, <tt>float</tt>, and <tt>double</tt>)
 *  to sequences of bits and writing them to standard output.
 *  Uses big-endian (most-significant byte first).
 *  <p>
 *  The client must <tt>flush()</tt> the output stream when finished
 *  writing bits.
 *  <p>
 *  The client should not intermixing calls to <tt>BinaryStdOut</tt>
 *  with calls
 *  to <tt>StdOut</tt> or <tt>System.out</tt>; otherwise unexpected
 *  behavior 
 *  will result.
 */
public final class BinaryStdOut {
    ...
    private static int buffer;     // 8-bit buffer of bits to write
                                   // out
    private static int N;          // number of bits remaining in
                                   // buffer
    ...
    // write out any remaining bits in buffer to standard output,
    // padding with 0s
    ...
    /**
     * Flush standard output, padding 0s if number of bits written so
     * far
     ...
     * @throws RuntimeException if <tt>x</tt> is not between 0 and
     * 2<sup>r</sup> - 1.
     */
